...
session_start();
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
					...
					if (!empty($cliente = $_POST['cliente']) && !empty($forma_de_pago = $_POST['method'])) {
							echo "procesando la orden de compra";
						// Comenzamos a procesar la orden de compra
		...
		foreach ($_SESSION['carrito'] as $key => $items) {
			...
			$arrayName [] = ['id_producto' => $items['item_id'],'cantidad' =>$items['item_loot'],'precio'=>$items['item_price'], 'id_factura'=> $id_factura];
							...
							var_dump($res = $venta->registrar_detalles_factura($arrayName));
							if ($res == TRUE) 
							{
